...
En lo que respecta a \text{dist_shop},se realizó la siguiente discretización para poder tener una aproximacón de su efecto sobre la variable price:
...
a continuación se presentá el gráfico de cajas del precio de oferta según \text{dist_shop} por \text{zona_avditalia} utilizando la discretización mencionada.
...
<<RF_imputmedia, eval=FALSE, message=FALSE, warning=FALSE, fig = TRUE>>=
############ RF 
