...
<<results=tex>>=
print(xtable(as.data.frame(RMSE_lm)))
@
...
A continuación se presentan los resultados del árbol de regresión ajustado sobre la variable price. En primer lugar se presenta el gráfico del modelo, luego de realizar el proceso de poda.
...
@
\captionof{figure}{Árbol de regression sobre la variable \text{price}. La primera partición es realizada con la variable \text{full\_bathrooms}. Las variables \text{dist\_rambla}, \text{total\_area}, \text{bedrooms} e \text{ingresomedio\_ech} son utilizadas en las siguientes particiones.}
\end{figure}
...
<<results=tex>>=
kable(rmse_rf, caption = 'Error cuadrático medio de los modelos ajustados por Random Forest', booktabs = T, col.names = c("Método de imputación","ECM"), format.args = list(big.mark = ".")) %>% kable_styling(latex_options = "HOLD_position") %>% row_spec(0,bold=TRUE)
@
...
<<eval=TRUE>>=
RMSE_boosting <- RMSE(predict.gbm(boosting_train, test), test$price)
RMSE_boosting_mr <- RMSE(predict.gbm(boosting_train, test_mr), test_mr$price)
...
kable(rmse_boost, caption = 'Error cuadrático medio de los modelos ajustados por Boosting', booktabs = T, col.names = c("Método de imputación","ECM"), format.args = list(big.mark = ".")) %>% kable_styling(latex_options = "HOLD_position") %>% row_spec(0,bold=TRUE)
@
...
<<eval=FALSE>>=
RMSE_svr <- RMSE(predict(SVR_train, test), test$price)
RMSE_svr_mr <- RMSE(predict.gbm(boosting_train, test_mr), test_mr$price)
...
kable(rmse_rf, caption = 'Error cuadrático medio de los modelos ajustados por SVR', booktabs = T, col.names = c("Método de imputación","ECM"), format.args = list(big.mark = ".")) %>% kable_styling(latex_options = "HOLD_position") %>% row_spec(0,bold=TRUE)
@
