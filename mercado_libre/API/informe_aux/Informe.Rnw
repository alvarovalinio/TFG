...
\begin{figure}
<<fig = TRUE, out.width = '70%', fig.align = 'c', fig.pos = 'h'>>=
...
@
\captionof{figure}{Histograma del precio de oferta de los apartamentos a la venta en Montevideo}
\end{figure}
...
@
\captionof{figure}{Gráficos de barras para diferentes variables cualitativas. En panel superior se encuentran a la izquierda la variable bedrooms y a la derecha la variable full_bathrooms. Por otro lado, en el panel inferior a la izquierda se encuentra la variable zona_avditalia y a la derecha has_swimming_pool}
\end{figure}
...
@
\captionof{figure}{Gráfico de caja de price según zona_avditalia y full_bathrooms}
\end{figure}
