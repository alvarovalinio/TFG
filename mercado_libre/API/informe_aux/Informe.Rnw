...
Por otra parte, una vez realizado el proceso de tratamiento de datos faltantes especificado en la sección \ref{secfalt} se mantiene un total de \Sexpr{paste(dim(aptos_sin_na)[2])} variables en el caso de imputación de valores faltantes por la media, y \Sexpr{paste(dim(aptos_mr)[2])} variables en el caso de imputación de valores faltantes por Random Forest. 
...
En ésta sección se presentan los principales resultados obtenidos luego de realizar la implementación de las técnicas detalladas en la sección \ref{MT}.
...
Luego, se exponen los resultados de la implementación de las técnicas de aprendizaje estadístico  explicitadas en la sección \ref{MT}.
...
Por último, en forma de resumen, se muestra un análisis comparativo de los diferentes modelos en función de su performance predictiva mediante la metodología de k-folds e hyperparameter tuning especificada en la sección \ref{cv}. En particular se trabajó con 5-folds.

De éste último, al mejor modelo se presenta el análisis gráfico de interpretabilidad según lo detallado en la sección \ref{interp}.

Se destaca que todas las etapas anteriores se aplicaron considerando las dos técnicas de imputación mencionadas en la sección \ref{secfalt}. 
...
Una vez observada las principales características de los modelos, se procedió a obtener cierta medida de la performance predictiva en función de las métricas definidas en la sección \ref{cv}. Las mismas se presentan a continuación en la tabla \ref{tab3}. 
...
Se destaca que el ajuste se realizó utilizando la función \textit{ranger} del paquete \textit{ranger} y tomando los valores por defecto de los hyperparametros detallados en la sección \ref{rf} para ambos métodos de imputación. 
...
Luego se presenta graficamente en la figura \ref{rf_import} un análisis de la importancia de las variables siguiendo la metodología detallada en la sección \ref{rf}. 
...
Por otro lado se procedió a obtener cierta medida de la performance predictiva en función de las métricas definidas en la sección \ref{cv}. Las mismas se presentan a continuación en la tabla \ref{tab3}.
...
Se destaca que el ajuste se realizó utilizando la función \textit{gbm} del paquete \textit{gbm} y tomando los valores por defecto de los hyperparametros detallados en la sección \ref{boosting} para ambos métodos de imputación. 
...
Luego se presenta graficamente en la figura \ref{boost_import} un análisis de la importancia de las variables siguiendo la metodología detallada en la sección \ref{boosting}. 
...
@
\captionof{figure}{Gráfico de importancia de las 10 primeras variables en los modelos ajustados por Boosting. Se observa que las variables cantidad de baños completos, área total e ingreso medio ECH son las 3 variables más importantes en ambos modelos. A su vez, en lo que respecta a las variables geoespaciales construidas, distancia a la rambla se encuentra entre las 10 variables más importantes para ambos casos. Por otro lado se destaca que, a la hora de realizar la imputación por Random Forest, la variable condición del ítem condition se encuentra entre las 10 variables más importantes.}
\end{figure}
\label{boost_import}

Por otro lado se procedió a obtener cierta medida de la performance predictiva en función de las métricas definidas en la sección \ref{boosting}. Las mismas se presentan a continuación en la tabla \ref{tab5}.
...
En éste sección se presentan los resultados del ajuste por Support Vector Regression considerando ambas metodologías de imputación. Se destaca que el ajuste se realizó utilizando la función \textit{ksvm} del paquete \textit{kernlab} y tomando los valores por defecto de los hyperparametros detallados en la sección \ref{svr} para ambos métodos de imputación. 

De esta manera, en primer lugar se presenta en la tabla \ref{hypsvr} los valores utilizados. 
...
En éste sección se presentan los resultados del ajuste por Suppport Vector Regression considerando ambas metodologías de imputación. 

En primer lugar se presenta graficamente un análisis de la importancia de las variables siguiendo la metodología detallada en la sección \ref{svr}. 
...
Por otro lado se procedió a obtener cierta medida de la performance predictiva en función de las métricas definidas en la sección \ref{svr}. Las mismas se presentan a continuación en la tabla \ref{tabsvr}.
...
Por último, a continuación se presenta en la tabla \ref{resmod} con los valores del RMSE, $R^2$ y MAE para cada uno de los modelos implementados.
...
Una vez realizado los ajustes de los modeos detallados en la sección \ref{MT} utilizando los valores por defecto, con el fin de mejorar la performance predictiva se procedió a realizar una búsqueda orientada sobre los posibles valores para los hiperparámetros de cada modelo. Para ello, se trabajó con la función \textit{train} del paquete \textit{CARET}. Se destaca que ésta función tiene especificada por defecto una grilla de valores para los hiperparámetros de cada modelo. Se realiza un ajuste para cada combinación de hiperparámetros en dicha grilla y se evalúan los ajustes en función de las métricas detalladas en la sección \ref{cv}.
