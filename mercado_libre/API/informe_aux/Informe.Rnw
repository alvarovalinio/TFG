...
load(here("mercado_libre/modelos/LM/Caret","LM_caret.RDS")) 

load(here("mercado_libre/modelos/LM/Caret","LM_caret_mr.RDS")) 
...
En este captiulo se presenta los aspectos metodologicos más relevante para realizar el trabajo. El mismo se encuentra conformado por cuatro secciones.

Donde, en la Sección \ref{sec:machinelearning} se detalla la metodología de los modelos ajustados. Luego en las Secciones \ref{sec:cv} y \ref{sec:interp}, se encuentra explicitada la metodología para evaluar la performance predicitva y realizar un analisis de interpretabilidad de los modelos ajustados respectivamente.

Por último, en la Sección \ref{sec:secfalt} se presenta la metodología llevada acabo para realizar el tratamiento de datos faltantes en la base de datos.

\section{Análisis Supervisado y Aprendizaje Estadístico\label{sec:machinelearning}}

Con el fin de obtener predicciones del precio de oferta de los inmuebles, fueron implementadas diferentes técnicas de aprendizaje estadístico. Estas consisten en modelar y analizar conjuntos de datos, mediante el aprendizaje de ejemplos, con el fin de predecir y estimar resultados en forma automática.
...
\subsection{Modelo de regresión lineal múltiple de precios hedónicos \label{subsec:mlMT}}
...
A pesar de que los árboles de regresión poseen un alto grado de interpretabilidad, estos poseen la gran limitante de ser inestables. Esto en el sentido de que pequeñas variaciones en el conjunto de entrenamiento y testeo generan grandes cambios en las estimaciones. (Hastie, 2001)
...
Esto último, con el fin de realizar un proceso de actualización de los residuos del modelo y por lo tanto mejorando la predicción del estimador en áreas donde el mismo no realiza un buen ajuste. (Hastie, 2001)

Generalmente, en \textit{Boosting} cada arbol suele estar conformado por pocas particiones, por lo que el procedimiento de aprendizaje suele ser "lento". (James, 2013) 
...
Donde, la letra $B$ denota el número de árboles utilizados en el algortimo. Se destaca que, a diferencia de \textit{Random Forest}, \textit{Boosting} puede generar un sobreajuste a los datos en caso que $B$ sea grande, a pesar de que éste sobreajuste ocurra de manera lenta. (James, 2013)
...
A su vez, el parámetro $d$ puede ser intepretado también como el parámetro que controla el orden de interacción entre los modelos, ya que las $d$ particiones pueden involucrar a los sumo $d$ variables. (James, 2013)

Dichos parámetros ($B$, $\lambda$ y $d$) se estiman mediante una metodologia de cross-validation.
...
Luego, se tiene que la solución al problema de minimización anteriormente mencionado, involucra el producto escalar entre las observaciones y no a las observaciones en si (Hastie, 2001). De esta forma, se puede re expresar a la función de regresión mediante la siguiente expresión:
...
Siendo $n_k$ es la cantidad de observaciones en la k-ésima muestra. 
...
Nuevamente, siendo $n_k$ es la cantidad de observaciones en la k-ésima muestra. 
Por otro lado, con el fin de obtener el modelo con la mejor performance predictiva, se realizó un proceso de \textit{hyperparameter tuning}. El mismo consiste en obtener la mejor combinación de hiperparámetros posibles mediante una metodología de cross-validation.

En este contexto, se define a un hyperparámetro como un valor necesario para ajustar un modelo el cual no se determina a partir de los datos sino que, por el contrario, es necesario que sea especificado previamente a la realización del ajuste. Dependiendo del algoritmo con el que se trabaje, el rol de los mismos puede variar. 
...
A pesar de que en la literatura existen diferentes métodos para llevar a cabo el análsis, se trabajó mediante tres aproximaciones. 

Como primera aproximación, se implemento un análisis sobre la importancia de cada predictor, mediante la metodología \texit{Permutation-based feature importance}. Luego, se realizó el análisis mediante los gráficos denominados \texit{Partial Dependece Plot} (PDP). 
...
Los gráficos denominados partial dependence plot (PDP) permiten observar el efecto marginal que una o dos variables tienen sobre la predicción de la variable de respuesta obtenida a través de un algoritmo de aprendizaje estadístico (Molnar, 2021). 
...
En dónde $X_C^{(i)}$ son los valores en la base de datos para las variables en las cuáles no son de interes y $n$ el número de observaciones.

Se destaca que PDP es un método que permite determinar de manera global la relación entre una o dos variables predictoras sobre la variable de respuesta. Asimismo, interesa destacar que uno de los supuestos de PDP es que las variables en $X_C$ y $X_S$ no están correlacionadas (Molnar, 2021).
...
Se destaca que el ajuste se realizó utilizando la función \textit{lm} del paquete \textit{stats}.
...
<<results=tex>>=
kable(lmt, caption = c("Principales resultados de los modelos lineales ajustados según el método de imputación utilizado, considerando muestra de entrenamiento y muestra de testeo."), escape = F, booktabs = T, align = "c", format.args = list(big.mark = "."), col.names = c('Método', 'RMSE', 'MAE', "$R^2$", "$R^2_a$","Estadístico F", "P-Valor" )) %>% kable_styling(latex_options = "HOLD_position", font_size = 10) %>% row_spec(0,bold=TRUE)
@
\label{tab2}

Según se observa en la Tabla \ref{tab2} los modelos lineales ajustados tienen un error de predicción de aproximadamente 43.000 y 30.000 dólares estadounidenses si se utiliza como medida de error a la raíz cuadrada del error cuadrático medio y error absoluto medio respectivamente. Asimismo, los modelos explican aproximadamente un 70 \% de la varianza en los datos y son globalmente significativos.
...
El gráfico de dispersión de la Figura \ref{figerrorml} sugieren la existencia de heteroscedasticidad en los residuos de los modelos ajustados. Por otra parte, el histograma de los residuos muestra que los mismos se encuentran en torno al valor 0.

Una vez realizado el análisis gráfico se procedió a realizar las pruebas sobre los supuestos de los residuos del modelo mencionadas en la sección \ref{subsec:mlMT}. Considerando ambos métodos de imputación y un nivel de significación del 0.05, se procedió a rechazar tanto el supuesto de normalidad como el supuesto de homoscedasticidad de los residuos en la medida que se rechazan las hipótesis nulas de ambas pruebas. Esto último teniendo en cuenta que en ambos casos el p-valor es menor al nivel de significación considerado. 
...
@
\captionof{figure}{Árbol de regresión obtenido al ajustar la variable precio de oferta en dólares mediante la base de datos construida a partir de información obtenida de Mercado Libre, una vez realizado el proceso de poda con un valor de CP igual a \Sexpr{cp_opt}. El método de imputación sobre valores faltantes es en éste caso es imputación por la media.}
\end{figure}
\label{figarb}

Según se observa en la Figura \ref{figarb} el árbol se conforma por \Sexpr{npart+1} nodos terminales (hojas). Los porcentajes dentro de cada nodo indican el porcentaje del número de observaciones que se encuentran en el mismo. Además, se explicita la predicción de las observaciones pertenecientes al nodo. A su vez, se destaca que se realizan \Sexpr{npart} particiones.
Asimismo, se tiene que la primera variable en realizar una partición binaria es la variable \text{full\_bathrooms}. Las variables \text{dist\_rambla}, \text{total\_area}, \text{bedrooms} e \text{ingresomedio\_ech} son utilizadas en las siguientes particiones.
Por otra parte, el gráfico de árbol realizando imputación de valores faltantes por Random Forest es presentado en la Figura \ref{figarbol} del anexo \ref{arbolA} debido a que no presenta diferencias sustanciales con el gráfico de árbol obtenido realizando imputación por la media.
...
<<results=tex>>=
kable(res_arbol, caption = 'Principales medidas de resumen de los árboles de regresión ajustados según metodología de imputación utilizada, considerando muestra de entrenamiento y muestra de testeo. Se presentan la raíz cuadrada del error cuadrático medio (RMSE), el coeficiente de determinación ($R^2$), y el error absoluto medio (MAE). ', booktabs = T, align = "c", col.names = c('Método', 'RMSE', "$R^2$", "MAE"), escape = F, format.args = list(big.mark = ".")) %>% kable_styling(latex_options = "HOLD_position", font_size = 12) %>% row_spec(0,bold=TRUE)
@
\label{tab3}

Como se observa en la Tabla \ref{tab3}, ambos modelos tienen un error promedio de predicción de aproximadamente 43.000 y 30.000 dólares estadounidenses si se utiliza como medida de error a la raíz cuadrada del error cuadrático medio y a el error absoluto medio respectivamente. Asimismo, los modelos explican aproximadamente un 70 \%  de la varianza en los datos.
...
Se destaca que el ajuste se realizó utilizando la función \textit{ranger} del paquete \textit{ranger} y tomando los valores por defecto de los hyperparámetros detallados en la sección \ref{subsec:rf} para ambos métodos de imputación. 
...
<<results=tex>>=
kable(hyp_rf, booktabs = T, align = "c", col.names = c('Hiperparámetro', 'Valor'), caption = 'Valores por defecto utilizados de los hiperparámentros en los modelos ajustados por Random Forest. Se detalla el número de árboles y la cantidad de variables utilizadas en cada árbol.' , escape = F, format.args = list(big.mark = ".")) %>% kable_styling(latex_options = "HOLD_position", font_size = 12) %>% row_spec(0,bold=TRUE)
@
\label{hyprf}
...
p1 <- importancia_rf$data %>% 
  ...
  geom_col(fill='mediumblue')+theme(legend.position="none")+labs(y="Variables",x="Importancia")
...
@
\captionof{figure}{Gráfico de importancia de las 10 primeras variables en los modelos ajustados por Random Forest. Se observa que las variables área cubierta, área total y distancia a la rambla son las 3 variables más importantes en ambos modelos.}
\end{figure}
\label{rf_import2}
...
<<results=tex>>=
kable(res_rf, caption = 'Principales medidas de resumen de los modelos ajustados por Random Forest según metodología de imputación utilizada, considerando muestra de entrenamiento y muestra de testeo. Se presentan la raíz cuadrada del error cuadrático medio (RMSE), el coeficiente de determinación ($R^2$), y el error absoluto medio (MAE), al igual que el error out of bag (RMSE OOB) y el coeficiente de determinación out of bag ($R^2$ OOB).', booktabs = T, align = "c", col.names = c('Método', 'RMSE', "$R^2$", "MAE", 'RMSE OOB', "$R^2$ OOB"), escape = F, format.args = list(big.mark = ".")) %>% kable_styling(latex_options = "HOLD_position", font_size = 11) %>% row_spec(0,bold=TRUE)
@
\label{tab4}

Según se observa en la tabla \ref{tab4} ambos modelos tienen un error promedio de predicción de aproximadamente 26.000 y 17.000 dólares estadounidenses si se utiliza como medida de error a la raíz cuadrada del error cuadrático medio  y al error absoluto medio respectivamente. Asimismo, los modelos explican aproximadamente un 82 \% de la varianza en los datos. Por su parte, si se consideran las métricas de RMSE y R2 out of bag, se tiene un error medio de predicción de aproximadamente 27.000 dólares estadounidenses y una varianza explicada aproximadamente del 88 porciento.
...
De ésta manera, en primer lugar se presenta en la Tabla \ref{hypb} los valores utilizados. 

<<>>=

hyp_boost <- tribble(
  ~Hyperparametro, ~Valor,
  "Número de árboles (B)",  round(boosting_train$n.trees), 
  "Tasa de aprendizaje (lambda)", boosting_train$shrinkage,
  "Número de particiones en cada arbol (d)",round(boosting_train$interaction.depth)
)
...
<<results=tex>>=
kable(hyp_boost, booktabs = T, align = "c", col.names = c('Hiperparámetro', 'Valor'), caption = 'Valores por defecto utilizados de los hiperparámentros en los modelos ajustados por Boosting. Se detalla el número de árboles, la tasa de aprendizaje y el número de particiones en cada árbol.' , escape = F, format.args = list(big.mark = ".")) %>% kable_styling(latex_options = "HOLD_position", font_size = 12) %>% row_spec(0,bold=TRUE)
@
\label{hypb}
...
@
\captionof{figure}{Gráfico de importancia de las 10 primeras variables en los modelos ajustados por Boosting. Se observa que las variables área total, cantidad de baños completos e ingreso medio ECH son las 3 variables más importantes en ambos modelos.}
\end{figure}
\label{boost_import}

En lo que respecta a las variables geoespaciales construidas, se observa en la Figura \ref{boost_import} que la variable distancia a la rambla se encuentra entre las 10 variables más importantes para ambos casos. Por otro lado se destaca que a la hora de realizar la imputación por Random Forest, la variable condición del ítem se encuentra entre las 10 variables más importantes.

Por otro lado se procedió a obtener cierta medida de la performance predictiva en función de las métricas definidas en la sección \ref{sec:cv}. Las mismas se presentan a continuación en la Tabla \ref{tab5}.
...
<<results=tex>>=
kable(res_boost, caption = 'Principales medidas de resumen de los modelos ajustados por Boosting según metodología de imputación utilizada, considerando muestra de entrenamiento y muestra de testeo. Se presentan la raíz cuadrada del error cuadrático medio (RMSE), el coeficiente de determinación ($R^2$), y el error absoluto medio (MAE).', booktabs = T, align = "c", col.names = c('Método', 'RMSE', "$R^2$", "MAE"), escape = F, format.args = list(big.mark = ".")) %>% kable_styling(latex_options = "HOLD_position", font_size = 11) %>% row_spec(0,bold=TRUE)
@
\label{tab5}

Según se observa en la Tabla \ref{tab5} ambos modelos tienen un error promedio de predicción de aproximadamente 40.000 y 39.000 dólares estadounidenses si se utiliza como medida de error a la raíz cuadrada del error cuadrático medio a el error absoluto medio respectivamente. Asimismo, los modelos explican aproximadamente un 60 porciento de la varianza en los datos.
...
@
\captionof{figure}{Gráfico de importancia de las 10 primeras variables en los modelos ajustados por Boosting. Se observa que las variables área cubierta, área total y distancia a la rambla son las 3 variables más importantes en ambos modelos.}
\end{figure}
\label{svrimport}

A su vez, en lo que respecta a las variables geoespaciales construidas, se observa en la Figura \ref{svrimport} que la variable distancia al shopping más cercano e ingreso medio ECH se encuentran entre las 10 variables más importantes para los ajustes según ambos métodos de imputación utilizados. Por otro lado se destaca que a la hora de realizar la imputación por Random Forest, la variable condición del ítem se encuentra entre las 10 variables más importantes.
...
<<results=tex>>=
kable(res_svr, caption = 'Principales medidas de resumen de los modelos ajustados por Support Vector Regression según metodología de imputación utilizada, considerando muestra de entrenamiento y muestra de testeo. Se presentan la raíz cuadrada del error cuadrático medio (RMSE), el coeficiente de determinación ($R^2$), y el error absoluto medio (MAE).', booktabs = T, align = "c", col.names = c('Método', 'RMSE', "$R^2$", "MAE"), escape = F, format.args = list(big.mark = ".")) %>% kable_styling(latex_options = "HOLD_position", font_size = 12) %>% row_spec(0,bold=TRUE)
@
\label{tabsvr}

La tabla \ref{tabsvr} muestra que ambos modelos tienen un error promedio de predicción de aproximadamente 37.000 y 17.000 dólares estadounidenses si se utiliza  como medida de error a la raíz cuadrada del error cuadrático medio y a el error abdolusto medio respectivamente. Asimismo, los modelos explican aproximadamente un 65 \% de la varianza en los datos.
...
res_tot <- tribble(
   ...
   "Modelo lineal", "Media", round(RMSE_lm), round(r2_lm,4) , round(MAE_lm),
  "Arbol de regresion", "Media", round(RMSE_arbol), round(r2_arbol,4), round(mae_arbol),
   "Random Forest", "Media", round(RMSE_rf), round(r2_rf,4), round(mae_rf),
    "Support Vector Regression", "Media",round(RMSE_svr), round(RMSE_svr), round(mae_svr),
   "Modelo lineal", "Random Forest", round(RMSE_lm_mr), round(r2_lm_mr,4), round(MAE_lm_mr),
   "Arbol de regresion",  "Random Forest", round(RMSE_arbol_mr),round(r2_arbol_mr,4), round(mae_arbol_mr),
   "Random Forest",  "Random Forest", round(RMSE_rf_mr), round(r2_rf_mr,4), round(mae_rf_mr),
    "Support Vector Regression",  "Random Forest", round(RMSE_svr_mr), round(r2_svr_mr,4), round(mae_svr_mr))
...
En la Figura \ref{fig30} se observa la evolución del error de predicción en función de los valores de los hiperparámentros para cada modelo. 

Se destaca que algunos de los hiperparámetros detallados en la sección \ref{cap:MT} permanecen constantes a la hora de realizar los diferentes ajustes utilizando las grillas por defecto. Los mismos se detallan en la Tabla \ref{hypdef}.
...
a <- RF_caret$results %>% filter(splitrule == 'variance') %>% 
      ggplot(aes(x = mtry, y = RMSE)) +
      geom_line(color = 'green2') +
      geom_point(color = 'green2') +
      xlab('Cantidad de variables incluidas aleatoriamente en cada árbol')
...
En lo que respecta a los ajustes realizados por Random Forest, en la Figura \ref{tun} se observa el error de predicción en función de la cantidad de variables incluidas de forma aleatoria en cada árbol, definidas en la grilla por defecto (2, 12, y 23). Se observa un comportamiento decreciente entre los valores 3 y 12 y un comportamiento creciente de menor magnitud, entre los valores 12 y 23. De esta forma, a la hora de realizar el proceso de hyperparameter tuning se consideraron todos los valores enteros entre los números 12 y 20.

Por otro lado, en cuanto a los ajustes realizados por Boosting, se observa que el error de predicción es decreciente conforme crece el número de árboles. Asimismo, se observa que el mismo es decreciente en cuanto al número de particiones. De esta manera, en la medida que los comportamientos observados para dichos hiperparámetros es decreciente conforme se incrementa el valor de los mismos, se tomaron valores del número de particiones entre 3 y 10; y cantidad de árboles entre 500 y 5000. Por último se decidió variar la tasa de parendizaje tomando valor 0.1 (valor fijo por defecto) y 0.01.
...
Se destaca que en la medida que los resultados obtenidos son similares para el caso de imputación de valores faltantes por Random Forest, la grilla seleccionada para realizar el proceso de hyperparameter tuning es análoga para cada modelo. En la Figura \ref{hyprfdef} se encuentran los gráficos correspondientes.

Un vez definida la grilla de hiperparámetros para cada modelo se procedió a implementar el proceso de hyperparameter tuning. En la sección \ref{hyptA} del anexo se presentan las tablas con los resultados de cada ajuste para cada modelo según la combinación de hiperámetros utilizada. 
...
<<results=tex>>=
kable(caret_tuning_row, caption = 'Tabla con las principales medidas de resumen para los modelos con mejor poder predictivos de cada algortimo utilizado luego de haber realizado el proceso de hyperparameter tuning', col.names=c('Algoritmo','Método de imputación','RMSE','$R^2$','MAE'),  escape = F, booktabs = T, align = "c", format.args = list(big.mark = ".")) %>% kable_styling(latex_options = "HOLD_position", font_size = 11) %>% row_spec(0,bold=TRUE) 
@
\label{carettuning}

En función de lo observado en la tabla \ref{carettuning}, para ambos métodos de imputación de valores faltantes, el modelo con mejor performance predicitiva con respecto a las tres métricas, es resultado de realizar un ajuste mediante el algoritmo Boosting, seguido por Random Forest. Por su contraparte, el modelo con peor performance predictiva con respecto a las tres métricas, es resultado de realizar un ajuste de regresión lineal.

Se destaca que en todos los casos los ajustes considerando metodología de imputación de valores faltantes por Random Forest tienen un poder predictivo superior. No obstante, como fue mencionado en la Sección estos ajustes incluyen adicionalmente a la variable condición del ítem. 

De esta forma, con el fin de seleccionar al mejor modelo en términos de performance predictiva y menor complejidad, en lo que respecta a la cantidad de variables utilizadas en el ajuste y el método de imputación de valores faltantes, se optó por el ajuste que es resultado de aplicar el algoritmo Boosting con método de imputación de valores faltantes por la media. 
...
<<results=tex>>=
kable(vars %>% select(-Fuente), caption = 'Descripción de las variables previo al proceso de depuración', booktabs = T, align = "c", longtable = TRUE, format.args = list(big.mark = ".")) %>% kable_styling(latex_options = c("hold_position"), font_size = 9) %>% row_spec(0,bold=TRUE) %>%
...
<<results=tex>>=
kable(p_na %>% filter(p_na > 0), caption = 'Proporción de valores faltantes por variable', booktabs = T, align = "c", col.names = c("Proporción por variable"), format.args = list(big.mark = ".")) %>% kable_styling(latex_options = "HOLD_position", font_size = 11) %>% row_spec(0,bold=TRUE)
@
\label{tab12}
...
<<results=tex>>=
kable(bp_t, caption = 'Resultados de la prueba de Breush-Pagam para el análisis de homoscedasticidad de los residuos', booktabs = T, align = "c", col.names = c("Método de imputación", "P-Valor de la prueba", "Decisión" ), format.args = list(big.mark = ".")) %>% kable_styling(latex_options = "HOLD_position", font_size = 11) %>% row_spec(0,bold=TRUE)
@
\label{tab13}
...
<<results=tex>>=
kable(lt_t, caption = 'Resultados de la prueba de Llliefors para el análisis de normalidad de los residuos', booktabs = T, align = "c", col.names = c("Método de imputación", "P-Valor de la prueba", "Decisión" ), format.args = list(big.mark = ".")) %>% kable_styling(latex_options = "HOLD_position", font_size = 11) %>% row_spec(0,bold=TRUE)
@
\label{tab14}
...
<<results=tex>>=
kable(a, caption = 'Grilla de hiperparámetros por defecto para los modelos ajustados por Random Forest. e destaca que en todos los casos la cantidad de árboles utilizada en el ajuste es 500. Asimismo, la cantidad de observaciones mínimas en un nodo terminal se mantiene constate en 5.', escape = F, booktabs = T, align = "c", col.names = c("Cantidad de árboles", "Min. obs.", "Cantidad de variables", "Regla de partición"), format.args = list(big.mark = ".")) %>% kable_styling(latex_options = "HOLD_position", font_size = 11) %>% row_spec(0,bold=TRUE) 
@
\label{grilla1}

<<results=tex>>=
kable(b, caption = 'Grilla de hiperparámetros por defecto para los modelos ajustados por Boosting.  Se destaca que el número de observaciones mínimas en cada nodo terminal se mantiene constante en 10', booktabs = T, align = "c", col.names = c("Número de árboles", "Tasa de aprendizaje", "Número de particiones en cada árbol"), format.args = list(big.mark = "."),  escape = F) %>% kable_styling(latex_options = "HOLD_position", font_size = 11) %>% row_spec(0,bold=TRUE) 
@
\label{grillab}

<<results=tex>>=
kable(c, caption = 'Grilla de hiperparámetros por defecto para los modelos ajustados por Support Vector Regression.', booktabs = T, align = "c", col.names = c("Complejidad del modelo", "Error (epsilon)", "Escala (gamma)"), format.args = list(big.mark = "."),  escape = F) %>% kable_styling(latex_options = "HOLD_position", font_size = 11) %>% row_spec(0,bold=TRUE) 
@
\label{grillac}


<<results=tex>>=
kable(d, caption = 'Principales medidas de resumen para los modelos ajustados por Random Forest según la grilla especificada en el proceso de hyperparameter tuning y considerando metodología de imputación de valores faltantes por la media. Se destaca que en todos los casos la cantidad de árboles utilizada en el ajuste es 500. Asimismo, la cantidad de observaciones mínimas en un nodo terminal se mantiene constate en 5.', escape = F, booktabs = T, align = "c", col.names = c("Algoritmo","Método","Cant. Variables","RMSE","$R^2$", "MAE"), format.args = list(big.mark = ".")) %>% kable_styling(latex_options = "HOLD_position", font_size = 11) %>% row_spec(0,bold=TRUE) 
@
\label{grillad}

<<results=tex>>=
kable(d_mr, caption = 'Principales medidas de resumen para los modelos ajustados por Random Forest según la grilla especificada en el proceso de hyperparameter tuning y considerando metodología de imputación de valores faltantes por Random Forest. Se destaca que en todos los casos la cantidad de árboles utilizada en el ajuste es 500. Asimismo, la cantidad de observaciones mínimas en un nodo terminal se mantiene constate en 5.', booktabs = T, align = "c", col.names = c("Algoritmo","Método","Cant. Variables","RMSE","$R^2$", "MAE"), format.args = list(big.mark = ".")) %>% kable_styling(latex_options = "HOLD_position", font_size = 11) %>% row_spec(0,bold=TRUE) 
@
\label{grilladmr}


<<results=tex>>=
kable(e, caption = 'Principales medidas de resumen para los modelos ajustados por Boosting según la grilla especificada en el proceso de hyperparameter tuning y considerando metodología de imputación de valores faltantes por la media. Se destaca que el número de observaciones mínimas en cada nodo terminal se mantiene constante en 10.', booktabs = T, align = "c", col.names = c("Algoritmo","Método","Particiones en cada árbol", "Cantidad de árboles", "Tasa de aprendizaje", "RMSE","$R^2$", "MAE"), format.args = list(big.mark = ".")) %>% kable_styling(latex_options = "HOLD_position", font_size = 11) %>% row_spec(0,bold=TRUE) 
@
\label{grillae}


<<results=tex>>=
kable(e_mr, caption = 'Principales medidas de resumen para los modelos ajustados por Boosting según la grilla especificada en el proceso de hyperparameter tuning y considerando metodología de imputación de valores faltantes por Random Forest. Se destaca que el número de observaciones mínimas en cada nodo terminal se mantiene constante en 10', booktabs = T, align = "c", col.names = c("Algoritmo","Método","Particiones en cada árbol", "Cantidad de árboles", "Tasa de aprendizaje", "RMSE","$R^2$", "MAE"), format.args = list(big.mark = ".")) %>% kable_styling(latex_options = "HOLD_position", font_size = 10) %>% row_spec(0,bold=TRUE) 
@
\label{grillaemr}

<<results=tex>>=
kable(f, caption = 'Principales medidas de resumen para los modelos ajustados por Boosting según la grilla especificada en el proceso de hyperparameter tuning y considerando metodología de imputación de valores faltantes por la media. Se destaca que el valor de epsilon para los ajuestes se mantiene constante en 0.1', booktabs = T, align = "c", col.names = c("Algoritmo", "Imputación", "Escala (gamma)", "Complejidad del modelo (C)", "RMSE", "$R^2$", "MAE") , format.args = list(big.mark = ".")) %>% kable_styling(latex_options = "HOLD_position", font_size = 10) %>% row_spec(0,bold=TRUE) 
@
\label{grillaf}

<<results=tex>>=
kable(f_mr, caption = 'Principales medidas de resumen para los modelos ajustados por Boosting según la grilla especificada en el proceso de hyperparameter tuning y considerando metodología de imputación de valores faltantes por Random Forest. Se destaca que el valor de epsilon para los ajustes se mantiene constante en 0.1', booktabs = T, align = "c",col.names = c("Algoritmo", "Imputación", "Escala (gamma)", "Complejidad del modelo (C)", "RMSE", "$R^2$", "MAE"), format.args = list(big.mark = ".")) %>% kable_styling(latex_options = "HOLD_position", font_size = 10) %>% row_spec(0,bold=TRUE) 
@
\label{grillafmr}
...
RF_caret_tunning_tot <-RF_caret_tunning$results %>%
      select(-RMSESD, -RsquaredSD, -MAESD) %>% mutate( n.trees = 500,
                                                       metodo_imput = 'Media') %>% 
...
SVR_caret_tunning_tot <- SVR_caret_tunning$results %>% mutate(epsilon  = 0.1,
                                                              metodo_imput = 'Media') %>% 
      relocate(metodo_imput, epsilon)

SVR_caret_tunning_mr_tot <- SVR_caret_tunning_mr$results %>% mutate(epsilon  = 0.1,
                                                                   metodo_imput = 'Random Forest') %>% relocate(metodo_imput, epsilon)
...
<<results=tex>>=
kable(bind_rows(RF_caret_tunning_tot, RF_caret_tunning_mr_tot), caption = 'Mejores modelos', booktabs = T, align = "c", format.args = list(big.mark = ".")) %>% kable_styling(latex_options = "HOLD_position", font_size = 11) %>% row_spec(0,bold=TRUE) 
@
\label{tab30}

<<results=tex>>=
kable(bind_rows(Boosting_caret_tunning_tot, Boosting_caret_tunning_mr_tot), caption = 'Mejores modelos', booktabs = T, align = "c", format.args = list(big.mark = ".")) %>% kable_styling(latex_options = "HOLD_position", font_size = 11) %>% row_spec(0,bold=TRUE) 
@
\label{tab31}

<<results=tex>>=
kable(bind_rows(SVR_caret_tunning_tot, SVR_caret_tunning_mr_tot), caption = 'Mejores modelos', booktabs = T, align = "c", format.args = list(big.mark = ".")) %>% kable_styling(latex_options = "HOLD_position", font_size = 11) %>% row_spec(0,bold=TRUE) 
@
\label{tab32}
