...
Siendo $(X_1, \cdots, X_p)$ el vector de las $p$ características asociadas y $(\beta_1, \cdots, \beta_p)$ el vector de los precios hedónicos. Es importante observar que el vector de precios hedónicos asociados a las características coincide con el vector de parámetros de un modelo líneal clásico.
...
El modelo de precios hedónicos puede ser generalizado para el caso no lineal.
...
<<fig = TRUE, fig.cap = "Histograma del precio de oferta (en logaritmos)">>=
...
<<fig = TRUE, fig.cap = "Cantidad de dormitorios">>=
...
<<fig = TRUE, fig.cap = "Precio de publicación según zona avenida italia">>=
...
<<fig = TRUE, fig.cap = "Precio de publicación según distancia a la rambla por nivel de ingreso">>=
aptos %>% 
...
<<fig = TRUE, fig.cap = "Precio de publicación según cantidad de baños completos">>=
aptos %>%
...
<<fig = TRUE, fig.cap = "Gráfico de Violin del precio de publicación (en logaritmos) según distancia al shopping más cercano">>=
aptos %>%
...
<<fig = TRUE, fig.cap = "Precio de publicación según distancia a un shopping y zona avenida italia">>=
aptos %>% 
      ggplot(aes(x=as.character(dist_shop), y=price, 
                 fill=as.character(dist_shop))) + 
      geom_violin() +
...
<<fig = TRUE, fig.cap = "Precio de publicación y distancia a la rambla por cantidad de habitaciones">>=
aptos %>%
...
<<RF_imputmedia, echo=FALSE, message=FALSE, warning=FALSE, fig = TRUE, eval = FALSE >>=
############ RF 
